...
<?php
    ...
    function verUsuarios()
    {
        ...
        $usuarios = $usuario_model->select('users.id, users.username, nombres, apellido_paterno, apellido_materno, group, telefono, telefono_emergencia')->join('users', 'users.id = users_information.user_id')->join('auth_groups_users', 'users.id = auth_groups_users.user_id')->findAll();
        $datos['estaLogeado'] = auth()->loggedIn();
        $datos['nombreUsuario'] = auth()->getUser()->username;
        $datos['idUsuario'] = auth()->getUser()->id;
    ...
    public function editarUsuario($user_id)
    {
        ...
        $datos['usuario'] = $usuario_model
            ->select('users.id, users.username, secret AS email, nombres, apellido_paterno, apellido_materno, documento_ci, telefono, telefono_emergencia, contacto_emergencia, direccion')
            ->join('users', 'users.id = users_information.user_id')
            ->join('auth_groups_users', 'users.id = auth_groups_users.user_id')
            ->join('auth_identities', 'users.id = auth_identities.user_id')
            ->where('users_information.user_id', $user_id)
            ->first();
        echo '<pre>';
        ...
        echo view('dashboard/templates/sidebar');
        echo view('dashboard/templates/breadcrumbs');
        echo view('dashboard/editar_usuario');
        echo view('dashboard/templates/footer');
    }
}
